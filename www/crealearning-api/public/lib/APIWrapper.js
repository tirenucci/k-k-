...
function doLMSInitialize()
{
   ...
   if (api == null)
   {
      _PrintError("Unable to locate the LMS's API Implementation.\nLMSInitialize was not successful.");
      return "false";
   }
...
function doLMSGetValue(name)
{
   ...
   else
   {
      ...
      if (errCode != _NoError)
      {
         ...
         _PrintError("LMSGetValue("+name+") failed. \n"+ errDescription);
         return "";
      }
      else
      {
...
function doLMSSetValue(name, value)
{
   ...
   else
   {
      ...
      if (result.toString() != "true" && !name.endsWith(".type")) //Un erreur est affichée sur cmi.interaction.n.type alors qu'il n'y a rien on la cache
      {
         var err = ErrorHandler();
      }
   }
...
function findAPI(win)
{
   while ((win.API == null) && (win.parent != null) && (win.parent != win))
   {
      ...
      if (findAPITries > 7)
      {
         _PrintError("Error finding API -- too deeply nested.");
         return null;
      }
...
function getAPI()
{
   ...
   if (theAPI == null)
   {
	   _PrintError("Unable to find an API adapter");
   }
   return theAPI
}
